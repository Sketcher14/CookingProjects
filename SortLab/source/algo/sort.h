...
concept ContiguousSortableRange = std::ranges::contiguous_range<RangeType> && std::ranges::sized_range<RangeType> &&
                                  Sortable<std::ranges::range_value_t<RangeType>>;

/** Sortable range with random access iterators */
template<typename RangeType>
concept RandomAccessSortableRange =
        std::ranges::random_access_range<RangeType> && Sortable<std::ranges::range_value_t<RangeType>>;
